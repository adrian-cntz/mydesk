...
async function userLoggedMiddleware(req, res, next) {
  ...
  if (sessionUserId) {
    ...
    sessionUser = await poolDB.query(
      `SELECT * FROM usuarios WHERE legajo = ${sessionUserId}`
    );
    ...
    legajoUser = await poolDB.query(
      `SELECT * FROM usuarios WHERE legajo = ${legajoInCookie}`
    );
  }
